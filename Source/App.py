...
def predict(n_clicks, age, gender, ethnicity, parental_education, study_time, absences, tutoring, parental_support, extracurricular, sports, music, volunteering, gpa):
    ...
    StudentDiscriptors = float(input_scaled[0]) + float(input_scaled[0]) + float(input_scaled[0]) + float(input_scaled[0])
    Activity = float(input_scaled[0]) + float(input_scaled[0]) + float(input_scaled[0]) + float(input_scaled[0])
